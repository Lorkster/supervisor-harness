...
At the end of step 2 the tool must be genuinely useful to a developer every morning, on its own, with **zero
model calls at runtime**. That property is the point of stopping here — do not compromise it.
...
- **Code calls VictoriaLogs over plain HTTP, not through MCP.** MCP tool results are text in a model context;
  these two milestones must make no model calls at all. Use the VictoriaLogs MCP for your own interactive
  exploration while developing if it helps — never in the shipped path.
- **Read-only.** The tool issues `/select/logsql/*` queries and nothing else.
...
- Python unless the repo you are working in clearly says otherwise. Standard library plus a HTTP client; keep
...
2. **Do not fetch the baseline per candidate.** 25 candidates × 7 days is 175 queries. Instead run the *same
   discovery aggregate* once per day offset — seven queries total — and join by fingerprint in code. You get the
   whole distribution for free. Reserve `pattern_match()` for drill-down: the reproduce-this-finding query shown
   in the report, and the "still live in the last hour" check.
